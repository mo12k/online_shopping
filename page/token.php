...
// Clean up expired tokens (good practice)
$_db->exec("DELETE FROM token WHERE reset_token_expire_at < DATE_SUB(NOW(), INTERVAL 10 SECOND)");
// ------------------------------------------------------------------
...
// 3. Find the user + valid (non-expired) token + correct email
$stm = $_db->prepare("
    SELECT c.*, t.reset_token_expire_at 
    FROM customer c
    JOIN token t ON c.customer_id = t.customer_id
    WHERE t.reset_token_hash = ?
      AND c.email = ?
      AND t.reset_token_expire_at > NOW()
");
